...
const DeckOrderList = ({ items, onAddItem, onDeleteItem }: DeckOrderListProps) => {
  ...
  const handleAddItem = () => {
    if (newItem.productName.trim()) {
      ...
      setNewItem({ productName: '', quantity: 1, colour: '', size: '' });
      setIsAdding(false);
    }
  };
    ...
    const htmlContent = `
      ...
      <body>
        ...
        <div class="footer">
          ...
          <p>Precision counting done right</p>
        </div>
      </body>
      </html>
    `;
          ...
          <div className="divide-y">
            {items.map((item) => (
              ...
              >
                ...
                <div className="col-span-2 text-muted-foreground">{item.colour || '-'}</div>
                <div className="col-span-2 text-muted-foreground">{item.size || '-'}</div>
